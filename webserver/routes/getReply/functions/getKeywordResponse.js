        ...
        session.run(query).then(function(result) {
            ...
            if (result.records.length === 0) {
                ...
                result.records.forEach((record) => {
                  // console.log('in keyword response '+record + JSON.stringify(record));
                    let field = record._fields;
                    let contentType = field[0][0];
                    // let content = {};
                    hasAtleastSomeContent = true;
                     field[1].map((value, index) =>{
                      // console.log('value in map stringify: ' + JSON.stringify(value));
                    ...
                    if(contentType === 'blog') {
                      ...
                      textArray.push(content);
                        ...
                        // resultObj[contentType] = {value: value[0], likes: value[1], dislikes: value[2]};
                ...
                console.log('object...........: ', JSON.stringify(resultObj));
                ...
                if (hasAtleastSomeContent) {
                  //  @Mayanka: If spell check done show this message
                    if (flag == 1) {
                        ...
                        console.log('hasAtleastSomeContent true: ', replyObj.value);
                    }
                  //  @Mayanka: no matching keyword found
                    else {
                        resultObj.value = replyForKeyword[Math.floor(Math.random() * replyForKeyword.length)];
                        console.log('hasAtleastSomeContent true: ', replyForKeyword[Math.floor(Math.random() * replyForKeyword.length)]);
                    }
                    ...
                    console.log('hasAtleastSomeContent false: ', answerNotFoundReply[Math.floor(Math.random() * answerNotFoundReply.length)]);
